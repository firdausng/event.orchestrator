...
public class KafkaCloudNativeMessageService
{
    ...
    public bool IsCloudEvent(Message<string?, byte[]> message) =>
        GetHeaderValue(message, SpecVersionKafkaHeader) is not null ||
    ...
    public CloudEvent ToCloudEvent(Message<string?, byte[]> message,
        CloudEventFormatter formatter, IEnumerable<CloudEventAttribute>? extensionAttributes)
    {
        ...
        else
        {
            ...
            var version = CloudEventsSpecVersion.FromVersionId(versionId)
                          ?? throw new ArgumentException($"Unknown CloudEvents spec version '{versionId}'",
                              nameof(message));
    ...
    public Message<string?, byte[]> ToKafkaMessage(CloudEvent cloudEvent, ContentMode contentMode,
        CloudEventFormatter formatter)
    {
        ...
        switch (contentMode)
        {
            case ContentMode.Structured:
                value = BinaryDataUtilities.AsArray(
                    formatter.EncodeStructuredModeMessage(cloudEvent, out var contentType));
    ...
    private static string? GetHeaderValue(MessageMetadata message, string headerName) =>
        Validation.CheckNotNull(message, nameof(message)).Headers is null
            ? null
            : message.Headers.TryGetLastBytes(headerName, out var bytes)
